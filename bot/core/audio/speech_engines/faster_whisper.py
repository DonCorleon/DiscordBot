...
class FasterWhisperSink(voice_recv.BasicSink):
    ...
    def transcribe_user(self, pcm_data: bytes, member: discord.Member):
        ...
        if len(mono_audio) < min_samples:
            logger.debug(f"[Guild {self.vc.guild.id}] faster-whisper: Skipping short audio for {member.display_name} ({len(mono_audio)} samples < {min_samples}, {len(mono_audio)/SAMPLE_RATE:.2f}s)")
            return
        ...
        # Check for empty audio after resampling (prevents faster-whisper crash)
        if len(audio_16k) == 0:
            logger.debug(f"[Guild {self.vc.guild.id}] faster-whisper: Empty audio after resample for {member.display_name}")
            return
